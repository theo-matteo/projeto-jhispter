...
import { sampleWithRequiredData } from '../emprestimo.test-samples';
...
describe('Emprestimo Management Component', () => {
  ...
  it('Should call load all on init', () => {
    ...
    expect(comp.emprestimos()[0]).toEqual(expect.objectContaining({ id: 30317 }));
  });

  describe('trackId', () => {
    it('Should forward to emprestimoService', () => {
      const entity = { id: 30317 };
      jest.spyOn(service, 'getEmprestimoIdentifier');
      const id = comp.trackId(entity);
      expect(service.getEmprestimoIdentifier).toHaveBeenCalledWith(entity);
